...
public class HabitacionController {
    ...
    @GetMapping("/find")
    public ResponseEntity<?> findHotelById(@RequestBody Optional<HabitacionRequest> habitacionRequest) {
        // Recomendacion:
    ...
    @PostMapping()
    public ResponseEntity<?> insertHabitacion(@RequestBody HabitacionRequest habitacionRequest) {
        logger.info(">insert" + habitacionRequest.toString());
        HabitacionResponse habitacionResponse;
        try {
            habitacionResponse = habitacionService.insertHabitacion(habitacionRequest);
        } catch (Exception e) {
            logger.error("Error inesperado", e);
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
        if (habitacionResponse == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ErrorResponse.builder().message("Habitacion not insert").build());
        }
        return ResponseEntity.ok(habitacionResponse);
    }   
}
